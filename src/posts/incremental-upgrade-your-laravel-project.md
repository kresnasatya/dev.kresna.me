...
In November 2024, when I was still working for Udayana University, we have more than 30 web apps that use the old Laravel version and also use CodeIgniter version 3.x. The deployment also not follow the conventions especially for the Laravel. Around 2 to 5 web apps that uses the new Laravel version and those follow the Laravel deployment rule. We only have 12 programmers (included myself). Each of us has their responsibility to handle web apps. Even one of us is take care hundreds of websites that using WordPress, Laravel, and CakePHP. That's crazy.

Now, with the situation my coworkers got the "ancient Laravel projects" to take care and I just create a Laravel Diff to inspect file changes in upgrade Laravel project, I see a chance to use this tool. Then, I persuade my coworkers to upgrade the Laravel version of those projects incrementally. I voluntarily involve myself in this project in order to test the functionallity Laravel Diff. Well, it's not good at first, because there's no strict checker in Laravel Diff when we want to upgrade from X version (source) to Y version (target). For example, when we want to upgrade Laravel version 5.1, we don't allowed to upgrade it directly to the latest version (version 11). We must upgrade it incrementally, from Laravel 5.1 to 5.2. Also, there's specific case where there's no Laravel version 5.9. So, the latest version in 5.x is 5.8. Then, the upgrade must be from 5.8 to 6.x. Another challenge was the third-party packages in those admission systems. It uses ixudra/curl for Http request, the laravel-fpdf wrapper for print PDF (you create PDF with coordinate NOT HTML which is really really horrible), and laravelcollective/html for blade utility (it's support until Laravel version 10). 

It's really hard when upgrade the web apps has a lot of codes and a lot of routes. So, I make a simple rules: 
...
The question is: Is supervisor approved it? Honestly not (at first time). Even my supervisor was surprised that we can upgrade those "ancient projects" into Laravel version 10. He even ask my coworker whether this upgraded version was already in production. My supervisor was have a plan to re-write the app from the scratch using the newest Laravel in a month (at least MVP). But, I'm worry with this plan. First, our resources are limited, we only 12 programmers. Each of us is busy with the apps that they responsible to. Second, our deadline is limited because our customers needs to deliver fast as soon as possible. Then, the realistic solution was incremental upgrade the project. Fortunately in March 2025, my supervisor use our works. So, our works are pay off. With incremental upgrade here are benefits we get:
...
Well, I don't know what is happening now in those admission systems because I no longer work there. I think that this admission systems can be merged into one admission system. Actually, if they want, they can keep going to incremental upgrade the project into the latest Laravel version by replace the Laravel Collective's Blade into the Laravel Blade. But, if they want to re-write from scratch now, I think this is okay too as long as they have their time.
